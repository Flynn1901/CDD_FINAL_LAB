...
//      );
      ...
      CSA_CLA #(.ADDER_WIDTH(ADDER_WIDTH)) 
      CSA_CLA_inst (
        .iopA(operandA),
        .iopB(operandB),
        .iCin(carry_in),
    ...
    // Also indicates overflow for subtraction
    reg overflow;
    ...
    always@(*)begin
        ...
        if(iCmd==0) begin
            // Addition operation
            rRes <= { regCout, regResult};
            // Optional: Detect overflow for addition
            // overflow = (iOpA[OPERAND_WIDTH-1] == iOpB[OPERAND_WIDTH-1]) && 
            //            (iOpA[OPERAND_WIDTH-1] != regResult[OPERAND_WIDTH-1]);
        end
        else if(iCmd==1) begin
            // Subtraction operation
            rRes <= {regCout, regResult};
            // Detect overflow for subtraction (A - B)
            // Overflow when: (A_sign != B_sign) && (Result_sign != A_sign)
            overflow = (iOpA[OPERAND_WIDTH-1] != iOpB[OPERAND_WIDTH-1]) && 
                      (iOpA[OPERAND_WIDTH-1] != regResult[OPERAND_WIDTH-1]);
        end
        else begin
